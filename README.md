...
本專案搭配的是我們 fork 修改過的 Mercury 數據機。這代表我們不只是在聊天程式外層加功能，也可以調整底層 ARQ 協定與音訊處理行為。相較於只使用原版數據機，fork 版可能帶來下列優勢：

- 更貼近短波實際鏈路的自適應速率選擇：數據機可依 SNR、重送情況、待傳資料量與近期 ACK 狀態，在較穩定時提高資料模式，在不穩定時退回較保守模式。
- 較好的通道使用效率：ARQ 可減少不必要的等待、輪詢與過早斷線，讓半雙工收發切換更適合聊天這類間歇性資料。
- 較低的應用層開銷：聊天資料使用精簡長度前綴格式，長訊息才視情況壓縮，避免每則短訊息都背負過大的格式成本。
- 更好的操作體驗：接收端能顯示傳輸進度，未壓縮文字可邊收邊顯示；閒置斷線與正在輸入狀態也能更符合人工打字節奏。
- 更容易針對實測修正問題：音效裝置取樣率、重取樣、Windows 音效裝置名稱、TX 音訊輸出音量等問題，都可以在 fork 版數據機內直接調整。

這些優勢不代表在所有短波條件下都會變快；弱訊號、多路徑、QRM、過高或過低音量、錯誤的音效裝置設定仍會限制實際效果。若要使用 fork 協定的改善，通訊雙方應使用相同或相容版本的 fork 版 Mercury 數據機；原版 Mercury 或其他未同步修改的數據機不保證能與 fork 協定互通。

## 速率表

Mercury Chat 下方狀態列顯示的 `TX` / `RX` 速率來自 Mercury 回報的 `BITRATE` / `TXBITRATE`。這裡的 bps 是底層 FreeDV 資料模式的名目位元速率，不等於聊天文字實際淨吞吐量；實際速度還會扣掉 ARQ 標頭、ACK、半雙工切換、PTT 延遲、重送、訊號品質與 UTF-8 文字位元組數。

| GUI 顯示 | FreeDV 模式 | 名目速率 | 每個數據機訊框酬載 | 主要用途 |
| --- | --- | ---: | ---: | --- |
| `Rate L1 980 bps` | DATAC1 | 約 980 bps | 510 位元組 | 最快資料模式；通常只在寬頻寬、SNR 佳、ACK 穩定且待傳資料較多時使用。 |
| `Rate L3 321 bps` | DATAC3 | 約 321 bps | 126 位元組 | 中速資料模式；寬頻寬連線常見的聊天傳輸模式。 |
| `Rate L4 87 bps` | DATAC4 | 約 87 bps | 54 位元組 | 最保守的資料模式；500 Hz 窄頻寬會固定使用，寬頻寬連線也會從這裡起步或在鏈路變差時退回。 |
| 控制訊框 | DATAC13 | 約 65 bps | 14 位元組 | CALL、ACCEPT、ACK、TURN、DISCONNECT、信標等控制用途，不是一般聊天酬載速率。 |

頻寬設定會影響可用模式：

- `500 Hz`：嚴格窄頻寬，聊天酬載維持在 DATAC4，因此常見顯示為 `Rate L4 87 bps`。
- `2300 Hz` / `2750 Hz`：允許 fork 版 ARQ 在 DATAC4、DATAC3、DATAC1 之間自適應切換。連線通常先從 DATAC4 開始，之後依 SNR、重送情況、待傳資料量與 ACK 穩定度升速或降速。
- 雙向各自判斷：A 到 B 與 B 到 A 的鏈路品質可能不同，所以兩邊 GUI 顯示的 TX/RX 速率不一定完全相同。
