...
Windows 套件會包含聊天程式、Qt 執行時期檔案與 fork 版 Mercury 數據機。Windows ARM64 套件中的聊天程式是 ARM64 版本；目前內含的 Mercury 數據機是 x64 版本，會透過 Windows 11 ARM64 的 x64 相容層執行。
...
這個 Linux 套件目前只包含 Mercury Chat 主程式，不包含 Mercury 數據機。請另外安裝或編譯 fork 版 Mercury 數據機，並在程式的 `Modem` 分頁設定 `Executable` 路徑。
...
下載後解壓縮。Windows 使用者執行 `mercury-chat.exe`；Linux 使用者執行 `./mercury-chat`。
...
10. 選擇頻寬：`BW2300`、`BW500` 或 `BW2750`。
11. 按 `Start Modem` 啟動數據機。
12. 按 `Connect TNC` 連上數據機的 TNC 控制埠與資料埠。

TNC 連上後，程式會自動送出 `MYCALL`、`BW...`、`CHAT ON`、`LISTEN ON`，讓站台進入可呼叫與可接收狀態。必要時也可以按 `Initialize` 手動重新套用站台設定。

## Modem 分頁

### Mercury Modem

- `Executable`：Mercury 數據機執行檔路徑。Windows 套件通常已經放在同一個資料夾內；Linux 需要自行指定或讓 `mercury` 出現在 `PATH`。
...
程式啟動 Mercury 時會依目前設定組出類似指令：

```sh
mercury -p 8300 -b 8100 -x coreaudio -i 145 -o 145 -k left
```
...
- `MYCALL <call>`：設定本機呼號。
- `BW2300`、`BW500`、`BW2750`：設定 ARQ 頻寬。
- `CHAT ON`：啟用聊天模式。
- `LISTEN ON`：允許接收呼叫。
- `CQFRAME <call> <bw>`：送出信標。
- `CONNECT <mycall> <theircall>`：建立 ARQ 連線。
- `DISCONNECT`：中斷 ARQ 連線。
- `BUFFER`：查詢數據機待送資料量。

Mercury Chat 不會強制指定 DATAC 模式，也不會覆蓋 Mercury 底層 ARQ 的自適應速率規則。它只傳送頻寬設定，並把 Mercury 回報的 `SN`、`BITRATE`、`TXBITRATE`、`BUFFER`、`PTT` 當成狀態資訊顯示。
