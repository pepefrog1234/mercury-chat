...
Mercury Chat 下方状态栏显示的 `TX` / `RX` 速率来自 Mercury 返回的 `BITRATE` / `TXBITRATE`。这里的 bps 是底层 FreeDV 数据模式的标称比特率，不等于聊天文本实际净吞吐量；实际速度还会扣掉 ARQ 头部、ACK、半双工切换、PTT 延迟、重传、信号质量和 UTF-8 文本字节数。

| GUI 显示 | FreeDV 模式 | 标称速率 | 每个调制解调器帧载荷 | 主要用途 |
| --- | --- | ---: | ---: | --- |
| `Rate L1 980 bps` | DATAC1 | 约 980 bps | 510 字节 | 最快数据模式；通常只在宽带宽、SNR 好、ACK 稳定且待发送数据较多时使用。 |
| `Rate L3 321 bps` | DATAC3 | 约 321 bps | 126 字节 | 中速数据模式；宽带宽连接常见的聊天传输模式。 |
| `Rate L4 87 bps` | DATAC4 | 约 87 bps | 54 字节 | 最保守的数据模式；500 Hz 窄带宽会固定使用，宽带宽连接也会从这里起步或在链路变差时退回。 |
| 控制帧 | DATAC13 | 约 65 bps | 14 字节 | CALL、ACCEPT、ACK、TURN、DISCONNECT、信标等控制用途，不是普通聊天载荷速率。 |

带宽配置会影响可用模式：

- `500 Hz`：严格窄带宽，聊天载荷维持在 DATAC4，因此常见显示为 `Rate L4 87 bps`。
- `2300 Hz` / `2750 Hz`：允许 fork 版 ARQ 在 DATAC4、DATAC3、DATAC1 之间自适应切换。连接通常先从 DATAC4 开始，之后根据信号 SNR、重传情况、待发送数据量和 ACK 稳定度升速或降速。
- 双向各自判断：A 到 B 与 B 到 A 的链路质量可能不同，所以两边 GUI 显示的 TX/RX 速率不一定完全相同。
